...
const readRawData = async (data: Array<any>): Promise<{ fieldNames: string[]; datas: any[] }> => {
  return new Promise(async (resolve, reject) => {
    // const errorCallback = error => {
    //   reject(new Error(error.code));
    // };
...
export const receiveImportCreate = async (content: any) => {
  try {
    const { fileName, type, scopeBrandIds, user, uploadType, fileType, data } = content;
